...
func init() {
	scriptRunfile, err := bazel.Runfile("gazelle/hex_metadata_to_json")
	if err != nil {
		log.Printf("failed to initialize hex_metadata_to_json: %v\n", err)
		os.Exit(1)
	}
	...
	go func() {
		...
		if err := cmd.Wait(); err != nil {
			log.Printf("failed to wait for hex_metadata_to_json: %v\n", err)
			os.Exit(1)
		}
	}()
}
...
func (p *hexMetadataParser) parse(configFilename string) (*hexMetadata, error) {
	parserMutex.Lock()
